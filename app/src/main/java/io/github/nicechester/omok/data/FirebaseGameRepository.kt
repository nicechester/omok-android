...
open class FirebaseGameRepository : GameRepository {
    ...
    override val currentRoom: StateFlow<GameRoom?> = _currentRoom
    ...
    override suspend fun joinOrCreateGame(gameId: String, playerName: String, timerDuration: Int): Boolean {
        ...
        return try {
            ...
            Log.e(tag, "joinOrCreateGame failed: ${e.message}", e)
            false
        }
    }

    protected open suspend fun createGame(ref: DatabaseReference, uid: String, playerName: String, timerDuration: Int) {
    ...
    override suspend fun forfeit() {
        ...
        try {
            ...
            val winnerUid = players.entries
                .firstOrNull { (k, v) -> k != uid && (v as? Map<String, Any>)?.get("color") != null }?.key
    ...
    override suspend fun resetForRematch() {
        val gameId = currentGameId ?: return
        val ref = gamesRef.child(gameId)
        try {
            val snapshot = ref.get().await()
    ...
    override fun listenToGame(gameId: String) {
    ...
    override suspend fun approveUndo() {
        val gameId = currentGameId ?: return
        val ref = gamesRef.child(gameId)
        try {
            val snapshot = ref.get().await()
            val dict = snapshot.value as? Map<String, Any> ?: return
            if (dict["status"] as? String != "playing") return
            dict["undoRequest"] as? Map<String, Any> ?: return
            val moveCount = ((dict["moveCount"] as? Number)?.toInt() ?: 0) - 1
    ...
    override suspend fun rejectUndo() {
        val gameId = currentGameId ?: return
        val ref = gamesRef.child(gameId)
        try {
            val snapshot = ref.get().await()
    ...
    override suspend fun autoPassTurn(gameId: String, expectedTurn: String, expectedTurnStartedAt: Long) {
        val ref = gamesRef.child(gameId)
        try {
            val snapshot = ref.get().await()
            val dict = snapshot.value as? Map<String, Any> ?: return
            ...
            val players = dict["players"] as? Map<String, Any> ?: return
            val winnerColor = if (expectedTurn == "black") "white" else "black"
            val winnerUid = players.entries
                .firstOrNull { (_, v) -> (v as? Map<String, Any>)?.get("color") as? String == winnerColor }?.key
    ...
    private fun parseGameRoom(gameId: String, dict: Map<String, Any>): GameRoom {
        ...
        val undoRequest = (dict["undoRequest"] as? Map<String, Any>)?.let {
            UndoRequest(
        ...
        val reaction = (dict["reaction"] as? Map<String, Any>)?.let {
            Reaction(
    ...
    protected fun buildBoardMap(raw: Map<String, Any>): Map<String, String> =
    ...
    private fun countOpenThree(board: Map<String, String>, row: Int, col: Int, color: String, dr: Int, dc: Int): Boolean {
        ...
        val fwdGap = fwdOpen && (fwdEndR + dr) in 0..14 && (fwdEndC + dc) in 0..14 && board["${fwdEndR + dr}_${fwdEndC + dc}"] == color
        ...
        val bwdGap = bwdOpen && (bwdEndR - dr) in 0..14 && (bwdEndC - dc) in 0..14 && board["${bwdEndR - dr}_${bwdEndC - dc}"] == color

        return (fwdCount + 1 + bwdCount) == 3 && fwdOpen && bwdOpen && !fwdGap && !bwdGap
    }
